...
class ShopDialogCreate(ui.ScriptWindow):
	...
	def __LoadQuestionDialog(self):
		...
		self.UI["board"].SetTitleName(localeInfo.SHOP_WINDOW_CREATE)
		self.UI["board"].Hide()

		self.UI["nameText"] = ui.MakeText(self.UI["board"], localeInfo.SHOP_NAME, 95, 30, None)
		...
		self.UI["selectText"].SetText(localeInfo.SHOP_TIMEOUT)
		...
		self.UI["create"].SetText(localeInfo.SHOP_CREATE)
		...
		self.UI["cancel"].SetText(localeInfo.CANCEL)
	...
	def CreateShop(self):
		if len(self.UI["nameEdit"].GetText()) <=0:
			self.PopupMessage(localeInfo.SHOP_NAME_EMPTY)
			return
		id=int(self.UI["select"].DropList.GetSelectedItem().value)
		if int(id) <=0:
			self.PopupMessage(localeInfo.SHOP_TIMEOUT_EMPTY)
			return
	...
	def Show(self):
		...
		for i in xrange(len(constInfo.shop_cost)):
			...
			name=localeInfo.SHOP_CREATE_NORMAL
			if item["time"]>0:
				name=str(item["time"])+" "
				if item["time"] == 1:
					if item["time_val"]==3600:
						name+=localeInfo.SHOP_CREATE_HOUR
					else:
						name+=localeInfo.SHOP_CREATE_DAY
				else:
					if item["time_val"]==86400:
						name+=localeInfo.SHOP_CREATE_DAYS
					else:
						name+=localeInfo.SHOP_CREATE_HOURS
	...
	def Load(self):
		...
		item = constInfo.shop_cost[id-1]
		if item["price"]>0:
			self.UI["price"].SetText(localeInfo.SHOP_CREATE_PRICE+localeInfo.NumberToMoneyString(item["price"]))
			self.UI["price"].Show()
		else:
			self.UI["price"].Show()
	def UnLoad(self):
...
class ShopEditWindow(ui.ScriptWindow):
	...
	def __Load_BindObject(self):
		...
		self.CloseButton.SetText(localeInfo.SHOP_EDIT_SHOP_CANCEL)
		self.CloseButton.SetEvent(ui.__mem_func__(self.Close))
		self.titleName.SetText(localeInfo.SHOP_EDIT_SHOP_WINDOW)
		...
		self.Edit["Board"].SetTitleName(localeInfo.SHOP_SELECT)
		...
		self.Edit["ChangePrice"].SetText(localeInfo.SHOP_CHANGE_PRICE)
		...
		self.Edit["Remove"].SetText(localeInfo.SHOP_REMOVE_ITEM)
	...
	def RemoveItem(self,slot):
		...
		self.pop.SetText(localeInfo.SHOP_REMOVE_ITEM_QUEST)
...
class ShopDialog(ui.ScriptWindow):
	...
	def LoadDialog(self):
		...
		self.UI["board"].SetTitleName(localeInfo.SHOP_WINDOW_NAME)
	...
	def Load(self,data):
		...
		gui["manage"].SetText(localeInfo.SHOP_MANAGE)
	...
	def ManageShop(self,data,force=0):
		self.EditBoardY=35
		self.AddEditOption("change_name",localeInfo.SHOP_CHANGE_NAME,lambda arg1=data: self.OnChangeButtonClick(arg1))
		self.AddEditOption("get_yang",localeInfo.SHOP_GET_YANG,lambda arg1=data: self.GetShopYang(arg1))
		self.AddEditOption("edit",localeInfo.SHOP_EDIT,lambda arg1=data["id"]: self.OnEditButtonClick(arg1))
		self.AddEditOption("close",localeInfo.SHOP_CLOSE,lambda arg1=data: self.CloseShop(arg1))
	...
	def UpdateSize(self):
		...
		self.UI["create_button"].SetText(localeInfo.SHOP_OPEN)
	...
	def CloseShop(self,shop):
		self.pop = uiCommon.QuestionDialog()
		self.pop.SetText(localeInfo.SHOP_CLOSE_QUEST%(shop["name"]))
	...
	def GetShopYang(self,shop):
		if int(shop["gold"]) <=0:
			self.PopupMessage(localeInfo.SHOP_NOT_EARNED_YANG)
			return
		self.pop = uiCommon.QuestionDialog()
		self.pop.SetText(localeInfo.SHOP_YANG_QUEST%(localeInfo.NumberToMoneyString(shop["gold"]),shop["name"]))
	...
	def OnChangeButtonClick(self,shop):
		...
		inputDialog.SetTitle(localeInfo.SHOP_ENTER_NEW_NAME)
	...
	def __ShowToolTip(self,shop):
		if self.tooltip:
			...
			self.tooltip.AutoAppendTextLine(localeInfo.SHOP_INFORMATION+str(shop["name"]), titleColor)
			self.tooltip.AppendSpace(5)
			self.tooltip.AutoAppendTextLine(localeInfo.SHOP_EARNED_YANG+str(localeInfo.NumberToMoneyString((shop["gold"]))), POSITIVE_COLOR)
			self.tooltip.AppendSpace(5)
			self.tooltip.AutoAppendTextLine(localeInfo.SHOP_REST_ITEMS+str(int(shop["sold"])), POSITIVE_COLOR)
			self.tooltip.AppendSpace(5)
			self.tooltip.AutoAppendTextLine(localeInfo.SHOP_SOLD_ITEMS+str(shop["items"]), POSITIVE_COLOR)
			self.tooltip.AppendSpace(5)
			self.tooltip.AutoAppendTextLine(localeInfo.SHOP_TIME_TO_CLOSE+str(localeInfo.SecondToDHM(int(shop["time"])-int(time.time()))), NEGATIVE_COLOR)
			self.tooltip.AppendSpace(5)
