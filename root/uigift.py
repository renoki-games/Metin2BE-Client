...
class GiftDialog(ui.ScriptWindow):
	...
	def LoadWindow(self):
		...
		self.UI["board"].SetTitleName(localeInfo.GIFT_WINDOW_NAME)
		...
		self.UI["get_button"].SetText(localeInfo.GIFT_GET_ALL)
		...
		## Prev Button
		self.UI["prevButton"] = ui.MakeButton(self.UI["board"], self.UI["board"].GetWidth()-160, Y ,localeInfo.GIFT_PREV_PAGE, "d:/ymir work/ui/public/", "small_button_01.sub", "small_button_02.sub", "small_button_03.sub")
		self.UI["prevButton"].SetText("<<")
		self.UI["prevButton"].SetEvent(ui.__mem_func__(self.PrepPage), -1)
		
		## Next Button
		self.UI["nextButton"] = ui.MakeButton(self.UI["board"], self.UI["board"].GetWidth()-65, Y , localeInfo.GIFT_NEXT_PAGE, "d:/ymir work/ui/public/", "small_button_01.sub", "small_button_02.sub", "small_button_03.sub")
		...
		if self.tooltipItem:
			...
			if int(slot) in constInfo.gift_items.keys():
				...
				if it["vnum"]==1:
					...
					self.tooltipItem.AppendDescription(localeInfo.GIFT_MONEY_DESC, 26)
				...
				self.tooltipItem.AppendTextLine("|cff18BB00|h[%s %s]|h|r" % (localeInfo.GIFT_FROM,it["give"]))
				self.tooltipItem.AppendSpace(5)
				self.tooltipItem.AppendTextLine("|cff18BB00|h[%s %s]|h|r" % (localeInfo.GIFT_REASON,it["reason"]))
				self.tooltipItem.AppendSpace(5)
			else:
				self.tooltipItem.ClearToolTip()
				self.tooltipItem.HideToolTip()
	...
	def Get(self,slot):
		if int(slot) in constInfo.gift_items.keys():
			...
			self.pop.SetText(localeInfo.GIFT_GET_QUEST%(item.GetItemName()))
	...
	def GetAll(self):
		self.pop = uiCommon.QuestionDialog()
		self.pop.SetText(localeInfo.GIFT_GET_QUEST_ALL)
